...
class CurriculumVersion(Base):
    ...
    created_by = Column(Enum("ai", "teacher", name="created_by_enum"), nullable=False)
    source = Column(Enum("parse", "edit", "regenerate", name="version_source_enum"), nullable=False)

    summary = Column(Text, nullable=False)
    difficulty_assessment = Column(String(50))
    notes_for_teacher = Column(Text)
    created_at = Column(DateTime, default=datetime.utcnow, nullable=False)
